...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!keystrokes || !Array.isArray(keystrokes)) {
      return NextResponse.json({ error: 'Invalid keystroke data' }, { status: 400 });
    }
    ...
    const SCALE_TOLERANCE = 0.2; // 20% of test duration
    const toleranceMs = Math.max(BASE_TOLERANCE, Math.round(expectedDurationMs * SCALE_TOLERANCE));
    ...
    for (let i = 0; i < Math.min(wordsTyped.length, expectedWords.length); i++) {
      ...
      if (typed === expected) {
        ...
        incorrectChars +=
          Math.abs(typed.length - expected.length) + (minLength - matchingChars) + 1;
      }
    }
    ...
    if (wpm < 0 || accuracy < 0 || accuracy > 100) {
      return NextResponse.json({ error: 'Invalid calculated statistics' }, { status: 400 });
    }

    // keystroke anti-cheat validation
    const expectedKeystrokes = correctChars + incorrectChars;
    const keystrokeRatio =
      keystrokes.length > 0 ? keystrokes.length / (expectedKeystrokes || 1) : 1;
    if (keystrokes.length > 0 && (keystrokeRatio < 0.3 || keystrokeRatio > 10.0)) {
      return NextResponse.json({ error: 'Invalid keystroke-to-character ratio' }, { status: 400 });
    }
    ...
    if (keystrokes.length >= 20 && backspaceRatio > 0.8) {
      return NextResponse.json({ error: 'Suspicious typing pattern' }, { status: 400 });
    }

    //Insert validated result into database
    const { data, error } = (await adminClient
      .rpc('insert_validated_typing_result', {
        p_user_id: user.id,
        p_wpm: wpm,
        p_accuracy: accuracy,
